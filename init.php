	...
	public static function defaultConstants()
	{
		...
		return array(
			...
			// we come accross an unusual sever environment where that isn't the case.
				...
				'TASK_OVERDUE_HOURS'	=> ( isset( $_SERVER['IPS_CIC'] ) ) ? 72 : 36,
			...
			// to change them.
				...
				'SUITE_UNIQUE_KEY'	=> ( isset( $_SERVER['IPS_CIC'] ) and preg_match( '/^\/var\/www\/html\/(.+?)$/i', __DIR__, $matches ) ) ? str_replace( '/', '', $matches[1] ) : mb_substr( md5( '77846' . '$Rev: 3023$'), 10, 10 ),
				'CACHEBUST_KEY'		=> mb_substr( md5( '77846' . '$Rev: 3023$'), 10, 10 ),
